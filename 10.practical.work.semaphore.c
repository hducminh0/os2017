...
void *consume_thread(){
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		printf("\nConsumed item %d: %c, %d, %c\nfirst = %d, last = %d\n", i, consumed_item->type, consumed_item->amount, 
		consumed_item->unit, first, last);
	}
}
...
{
	pthread_t tid, tid2;
	sem_init(&semaphore, 0, BUFFER_SIZE);	//init semaphore with size of the Buffer, 0 indicate semaphore shared btwn thread
